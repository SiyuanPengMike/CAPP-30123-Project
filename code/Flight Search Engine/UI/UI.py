...
def route_suggest(Org, Des, connect_dict, time_max = 180, time_min = 120):
	...
	if (Org, Des) in direct:
		...
		if sort_suggest:
			...
			suggest = list('Flight Number: {}\n Depart at {}:{}\n Arrive at {}:{}'.format(i[0], i[2][:2], i[2][2:], i[3][:2], i[3][2:]) for i in sort_suggest[:idx])
	...
	if sort_full_list:
		...
		suggest2 = list('{} to {} to {}:\n Flight Number: {} & {}'.format(i[0][0],i[0][1],i[0][2], i[1], i[2]) for i in sort_full_list[:idx2])
...
def search():
    ...
    x3 = time_min.get()
    x4 = time_max.get()
    if x3.isdigit() and x4.isdigit():
        best_route, best_pair = route_suggest(Org = x1.upper(), Des = x2.upper(), connect_dict = connect_dict, time_max = int(x4), time_min = int(x3))
    else:
        best_route, best_pair = 'No flights available', None
    zs = []
    if best_route == 'No flights available':
        zs.append('No flights available')
    else:
        for string in best_pair:
            zs.append('{}: {}'.format(best_route, string))
    if Org.get() and Des.get() and x3.isdigit() and x4.isdigit():
        for idx, z in enumerate(zs):
            Label(root1, text=z, fg='blue', width=100, height=8, anchor='c').grid(row = idx)
    else:
        Label(root1, text='please enter correct and full information', fg='blue', width=50, height=20, anchor='c').pack()
label=Label(root,text='Orgin IATA code:',anchor='c').grid(row=0)
Org=Entry(root)
Org.grid(row=0,column=1)
label2=Label(root,text='Destination IATA code:',anchor='c').grid(row=1)
Des=Entry(root)
Des.grid(row=1,column=1)
label3=Label(root,text='Minimum Transition Time (minutes):',anchor='c').grid(row=2)
time_min=Entry(root)
time_min.grid(row=2,column=1)
label4=Label(root,text='Maximum Transition Time (minutes):',anchor='c').grid(row=3)
time_max=Entry(root)
time_max.grid(row=3,column=1)
Button(root,text='Search',anchor='c',width=6,height=1,command = search).grid(row=4,column=0)
Button(root,text='Exit',anchor='c',width=6,height=1,command = root.quit).grid(row=4,column=1)
root.mainloop()
